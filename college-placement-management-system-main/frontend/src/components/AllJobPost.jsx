...
import { getApplyDestination } from '../utility/jobActions';
...
function AllJobPost() {
  ...
  const handleApplyAction = (job) => {
    if (!job?._id) return;

    const destination = getApplyDestination(job);

    if (destination.type === 'external') {
      setToastMessage(`Opening the application page for ${job?.jobTitle || 'this role'}.`);
      setShowToast(true);
                            ...
                            <td>
                              <div className="flex flex-wrap items-center justify-end gap-2">
                                ...
                                {currentUser.role === 'student' && (
                                  <button
                                    type="button"
                                    onClick={() => handleApplyAction(job)}
                                    className={`rounded-full px-3 py-1.5 text-xs font-semibold transition ${isMatched ? 'bg-emerald-600 text-white hover:bg-emerald-700' : 'bg-blue-600 text-white hover:bg-blue-700'}`}
                                  >
                                    <i className="fa-solid fa-arrow-up-right-from-square mr-1" />
                                    {isMatched ? 'Applied' : 'Apply'}
                                  </button>
                                )}
                                {currentUser.role !== 'student' && (
                                  <>
