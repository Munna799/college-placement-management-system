...
const StudentDataYearBranchWise = async (req, res) => {
  try {
    ...
    const unassignedStudents = await User.find({
      role: "student",
      $or: [
        { "studentProfile.department": { $exists: false } },
        { "studentProfile.department": null },
        { "studentProfile.year": { $exists: false } },
        { "studentProfile.year": null },
      ],
    });

    return res.json({ firstYearComputer, firstYearCivil, firstYearECS, firstYearAIDS, firstYearMechanical, secondYearComputer, secondYearCivil, secondYearECS, secondYearAIDS, secondYearMechanical, thirdYearComputer, thirdYearCivil, thirdYearECS, thirdYearAIDS, thirdYearMechanical, fourthYearComputer, fourthYearCivil, fourthYearECS, fourthYearAIDS, fourthYearMechanical, unassignedStudents });
  } catch (error) {
